      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                <li v-for="(item,key) in addressListFilter" :key="key" :class="{'check':checkey==key}" @click="checkey=key;SAddrId=item.addressId">
                  <dl>
                    <dt>{{item.userName}}</dt>
            ...
            <div class="shipping-addr-more">
              <a class="addr-more-btn up-down-btn" href="javascript:;" @click="expand" :class="{'open':limt>3}">
                more
                <i class="i-up-down">
          ...
          <div class="next-btn-wrap">
            <router-link class="btn btn--m btn--red" :to="{path:'orderconfirm',query:{'addressId':SAddrId}}">Next</router-link>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed:{
    addressListFilter(){
      return this.addressList.slice(0,this.limt);
    }
  },
  mounted(){
    this.init()
  ...
  methods:{
    ...
    expand(){
      if(this.limt==3){
        this.limt=this.addressList.length;
      }else{
        this.limt=3
      }
    },
    setDefault(addressId){
      axios.post('/users/setdefault',{addressId:addressId}).then(()=>{})
        this.init()
    },
    closeModal(){
      this.isMdShow=false;
