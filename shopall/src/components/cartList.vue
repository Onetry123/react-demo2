              ...
              <li v-for="(item,key) in cartList" :key="key">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" :class="{'check':item.checked=='1'}" @click='editcart(item,"updatacheck")'>
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="editcart(item,false)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click="editcart(item,true)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="cart-tab-4">
                  ...
                  >{{parseInt(item.productNum)*parseInt(item.salePrice)}}</div>
                </div>
...
export default {
  ...
  data() {
   ...
   computed:{
    checkAllFlag(){
      return this.cheeckedCount==this.cartList.length
    },
    cheeckedCount(){
      let i=0;
    ...
    allPrice() {
      let num = 0;
      this.cartList.map((v, i) => {
        if(v.checked==1){
          num += parseInt(v.productNum) * parseInt(v.salePrice);
        }
      });
      return num;
    },
  ...
  methods: {
    getCartlist() {
      axios.get("/users/cartList").then(msg => {
        let res = msg.data;
    ...
    toggleCheckAll(){
      ...
      axios.post('/users/checkAll',{checkAll:flag}).then((res)=>{
      })
    },
    checkOut(){
      if(this.cheeckedCount>0){
          this.$router.push({
              path:'/address'
          })
      }
