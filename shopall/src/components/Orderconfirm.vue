              ...
              <ul class="cart-item-list">
                <li v-for="(item,key) in orderList" :key='key'>
                  ...
                  <div class="cart-tab-4">
                    <div class="item-price-total">{{(parseInt(item.productNum)*parseFloat(item.salePrice))|currency('$')}}</div>
                  </div>
                </li>
              </ul>
            </div>
  ...
  methods:{
    ...
    subTotal(){
      ...
      this.orderList.map((item)=>{
        num+=parseInt(item.productNum)*parseFloat(item.salePrice)
      })
      return num
    },
    payMent(){
      let addressId=this.$route.query.addressId
      let orderTotal=this.subTotal()+this.Shipping-this.discount+this.tax
      axios.post('/users/payMent',{addressId:addressId,orderTotal:orderTotal}).then((res)=>{
